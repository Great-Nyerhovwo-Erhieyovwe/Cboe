...
async function fetchWalletConfig() {
    try {
        // 🔑 UPDATE 1: Prepend API_BASE_URL
        const response = await fetch(`${API_BASE_URL}/api/config/wallet`);
        if (!response.ok) {
...
async function fetchUserProfile() {
    try {
        ...
        if (userProfile.isBanned === true) {
            ...
            // 🔑 UPDATE 3: Prepend API_BASE_URL for logout
...
async function fetchTransactions() {
    ...
    try {
        // 🔑 UPDATE 4: Prepend API_BASE_URL
        const response = await fetch(`${API_BASE_URL}/api/transactions/latest`); // Server returns transactions for current user
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
...
async function handleWithdrawSubmit(e) {
    ...
    try {
        ...
        // 🔑 UPDATE 6: Prepend API_BASE_URL
        const response = await fetch(`${API_BASE_URL}/api/transactions/withdrawal`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                amount: amount,
